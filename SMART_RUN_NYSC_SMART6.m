%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the main code for running the SMART Robot                 %
% Author: WVU                                                       %
...
biasRemovalCounts = 5/Ts_Desired;           % number of loop cycles to collect bias data
...
for i=1:Total_Steps
    ...
    elseif i==biasRemovalCounts
        ...
        X_Last = 0.457;
        Y_Last = 1.981;
        Bias_P = mean(SD.P(1:i));
        Bias_Q = mean(SD.Q(1:i));
        Bias_R = mean(SD.R(1:i));
    else                                                                      % If i>1
    ...
    if i<biasRemovalCounts
        ...
        if avoiding
            ...
            AngleVector = 10*(1:25)*pi/180;
            X_Curr = SD.X(i);
            Y_Curr = SD.Y(i);
            Euclid = sqrt((X_Curr-X_Last)^2+(Y_Curr-Y_Last)^2);
            if Euclid <= 2.5 && Iter >= 1 && CodeState < 3
                CodeState = 3;
            end
            ...
            switch CodeState
                case 1
                    [avoidDone, driveData] = TurnAngle(10*pi/180, S_Create, SD, driveData, i);
                    if avoidDone
                ...
                case 2
                    ...
                    if j > 25
                        ...
                        DistanceVector(j) = SD.Laser_RF(i);
                        YawVector(j) = mod(SD.Yaw(i),sign(SD.Yaw(i))*2*pi);
                        CodeState = CodeState-1;
                        j = j+1;
                    end
                case 3
                    %% Find target heading
                    Iter = Iter+1;
                    Theta_Tar = atan2((goalY-SD.Y(i)),(goalX-SD.X(i)));
                    
                    %% Find the best heading
                    [Dist_Peaks,idx]=max(DistanceVector);
                    Angle_Peaks=YawVector(idx);
                    [~,idx] = min(abs(Angle_Peaks-Theta_Tar));
                    MaxDist=Dist_Peaks(idx);
                    Theta_Head = Angle_Peaks(idx);
                    
                    %% Find current orientation
                    Theta_Curr = mod(SD.Yaw(i),sign(SD.Yaw(i))*2*pi);
                    
                    %% Find Discrepency and Rotate
                    Discrepency = Theta_Head-Theta_Curr;
                    CodeState = CodeState+1;
                case 4
                    [avoidDone, driveData] = TurnAngle(Discrepency, S_Create, SD, driveData, i);
                    if avoidDone
                        CodeState = CodeState+1;
                        avoidDone = 0;
                    end
                case 5
                    ...
                    CodeState = CodeState+1;
                case 6
                    [avoidDone, driveData] = DriveDistance(0.4, S_Create, SD, driveData, i);
                    if avoidDone
                        CodeState = CodeState+1;
                        avoidDone = 0;
                    end
                case 7
                    %% Move for Unit length
                    driveData = ResetDriveState(driveData);
                    CodeState = 1;
            end
            if SD.RF_F(i)<avoidTriggerDistance || SD.BumpFront(i) || SD.BumpLeft(i) || SD.BumpRight(i)
